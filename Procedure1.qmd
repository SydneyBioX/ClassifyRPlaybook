...
library(svglite)
svglite("~/ClassifyRPlaybook/publishFigures/Figure2a.svg", height = 3, width = 3.5)
performancePlot(classifyResult, fontSizes = c(7, 6, 5, 5))
dev.off()
```
...
```{r echo = FALSE, warning = FALSE}
library(svglite)
svglite("~/ClassifyRPlaybook/publishFigures/Figure2b.svg", height = 3, width = 3.5)
samplesMetricMap(classifyResult, showXtickLabels = FALSE, fontSizes = c(7, 6, 5, 5, 5))
dev.off()
```
