...
ClassifyR implemenents a variety of ways to evaluate the classifiability of samples from multiple views. A view could be an omics assay or it could be a particular metafeature generation of a particular data set.

## Introduction

In this case study, both different assays and different metafeatures will be demonstrated of the widely-known METRBEIC breast cancer data set. These have been pre-generated. Briefly,
...
The number of features varies from ten to 2000 for 165 patients. Next, look more closely at clinical data.
...
`timeRFS` and `eventRFS` columns will form the outcome to predict.
...
Examine whether any view alone is the best predictor or whether combinations of view adds value to accuracy. Note that any parameters to the `prepare` element of `extraParams` will be passed to the `prepareData` function.

### Indivdual Assays (2 minutes)

The first analysis fits a penalised Cox proportional hazards model to each assay alone. Remember that the default sceheme of `crossValidate` is 5-fold cross-validation repeated 20 times.
...
It is also worthwhile looking at per-sample C-index, which is simply the number of concordant pairs divided by the total number of feasible pairs of sample comaparisons between the predicted risk score and the event-time recorded outcome.
